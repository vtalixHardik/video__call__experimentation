...
import { connectSocket, joinRoom, listenForJoinSuccess, sendOffer, listenForAnswer, listenForErrors, listenForOffer, sendAnswer, sendICE, listenForICE, disconnectSocket, listenForUserLeft, resetSockets, saveChats, saveNotes } from "../services/socket";
import sendButton from "@assets/images/send__white.svg";
import mic__controls from "@assets/images/mic__control.svg";
import video__controls from "@assets/images/video__control.svg";
import end__call from "@assets/images/end__call.svg";
import { useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";

const ThirdRoom = ({ end_time, setIsOpen, appointmentId, setAppointmentComplete }) => {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			if (currentTime >= endDateTime) {
				...
				alert("Thank you for completing the 1-hour session. Any additional time beyond this is considered voluntary.");
			}
	...
	const endCall = () => {
		...
		saveChats(appointment_id, chats, token);
		saveNotes(appointment_id, notes, token);
	...
	const handleReceiveMessage = (e) => {
		...
		messagesRef.current.push({ isMe: false, message: e.data, old_text: true });
	...
	const createPeer = (id) => {
		...
		const peer = new RTCPeerConnection({
			iceServers: [
				...
				// {
      			// 	urls: "turn:turnserver.vtalix.com:3478", // Replace with your coTURN server's IP and port
      			// 	username: "turn1", // Optional: Replace with your coTURN username
      			// 	password: "PassWD1"  // Optional: Replace with your coTURN password
    			// },
	...
	useEffect(() => {
		...
		const addCandidates = async () => {
			...
			if (receivedICECandidates?.current?.length !== 0 && peerRef?.current?.signalingState === "stable" && answerSent === true) {
	...
	const sendMessage = (e) => {
		...
		setMessages(messages => [...messages, { isMe: true, user_id: user.id, message: text }]);
		messagesRef.current.push({ isMe: true, user_id: user.id, message: text });
		setText("");
	}
							...
							focus === "chat" && (
								<form className="flex flex-col w-full text-white h-full" onSubmit={sendMessage}>
									<div className="grow overflow-x-hidden w-full">
										<div className="p-2 h-dvh max-h-[calc(100vh_-_11.5rem)] overflow-y-auto overflow-x-hidden no__scrollbar text-wrap object-contain">
											{messages?.map((item, index) => (
												<div className={`w-fit max-w-[80%] text-wrap break-all my-1.5 p-3 text-black h-fit ${(!!(item.user_id) && item?.user_id === user.id) ? "bg-[#E6E8ED] ml-auto rounded-l-2xl rounded-tr-2xl" : "bg-[#FFD5B3] mr-auto rounded-r-2xl rounded-tl-2xl"}`} key={`${index}_${item?.message}`}>
									...
									<div className="flex w-full h-fit gap-3 justify-between items-center">
										...
										<button type="submit" className="min-w-fit cursor-pointer rounded-full  hover:bg-primary-lightOrange p-2 aspect-square bg-primary-orange hover:text-blue-600 border border-solid border-primary-orange">
											<img src={sendButton} alt="" className="text-red-900" />
										</button>
									</div>
								...
								<>
									<div className="w-full grow p-2">
										...
										<button
											...
											onClick={() => saveNotes(appointment_id, notes, token)}
										>
											Save Notes
										</button>
									</div>
