...
const ThirdRoom = ({ end_time }) => {
	...
	const handleTrackEvent = (e) => {
		...
		const waitForRemoteVideo = setInterval(() => {
			...
			console.log(
				"remote video.current is 2",
				remoteVideo.current || "Nil"
			);
		}, 100);
	};
	...
	const createPeer = (id) => {
		...
		peer.oniceconnectionstatechange = () => {
			...
			if (peer.iceConnectionState === "connected") {
				console.log(
					"✅ Connection stabilized! Sending last ICE candidate..."
				);
			}
		};
	...
	const handleNewICECandidate = async (candidate) => {
		console.log(
			"Peer connection while handling ICE candidates is ",
			peerRef.current
		);
		if (peerRef?.current?.iceConnectionState === "connected") {
			...
			console.log(
				"receivedICECandidatesState is ",
				receivedICECandidatesState
			);
	...
	useEffect(() => {
		...
		const addCandidates = async () => {
			console.log(
				"recieved candidates are ",
				receivedICECandidates?.current
			);
			console.log(
				"total candidates are ",
				receivedICECandidates?.current.length
			);
			console.log(
				"signalling state is ",
		...
		addCandidates();
		console.log(
			"Updated receivedICECandidatesState:",
			receivedICECandidatesState
		);
						...
						{focus === "chat" && (
							...
							>
								<div className="grow overflow-x-hidden w-full">
									<div className="p-2 h-dvh max-h-[calc(100vh_-_11.5rem)] overflow-y-auto overflow-x-hidden no__scrollbar text-wrap object-contain">
										{messages?.map((item, index) => (
											<div
												className={`w-fit max-w-[60%] text-wrap break-all my-1.5 p-3 text-black h-fit ${
													item?.isMe === true
								...
								<div className="flex w-full h-fit gap-3 justify-between items-center">
									...
									<button
										type="submit"
										className="cursor-pointer rounded-full bg-blue-600 p-2 aspect-square hover:bg-white hover:text-blue-600 border border-solid border-blue-600"
									>
										O
									</button>
								</div>
