...
const SecondRoom = () => {
	try {
		...
		const initializePeerConnection = async () => {
			...
			console.log("peer connection is ", peerCon);
			...
			//   });
			// };
			// peerCon.addEventListener("track", (ev) => {
			//   const vidTag = document.getElementById("remote-video");
			//   console.log("event.streams is ", ev.streams);
			//   console.log("vide tag is ", vidTag);
			//   vidTag.srcObject = ev.streams[0];
			//   vidTag.onloadedmetadata = () => {
			//     console.log("playigng the video now on loading");
			//     vidTag.play().catch((err) => {
			...
			peerCon.addEventListener("track", (ev) => {
				console.log("Received remote track event:", ev);
				...
				console.log(
					"Remote video srcObject set before:",
					remoteVideoRef.current.srcObject
				);
				// Ensure srcObject is set properly
				if (
					!remoteVideoRef.current.srcObject ||
					remoteVideoRef.current.srcObject.id !== remoteStream.id
				) {
					console.log(
						"!remoteVideoRef.current.srcObject || remoteVideoRef.current.srcObject.id !== remoteStream.id"
					);
					remoteVideoRef.current.srcObject = remoteStream;
				}

				console.log(
					"Remote video srcObject set after:",
					remoteVideoRef.current.srcObject
				);

				remoteVideoRef.current.onloadedmetadata = () => {
					console.log(
						"Remote video metadata loaded. Attempting to play..."
					);
					remoteVideoRef.current.play().catch((err) => {
						console.error(
							"Error trying to play remote video:",
							err
						);

						// If autoplay fails, try muting/unmuting trick
				...
				if (!remoteStream.active) {
					console.warn("Remote stream is inactive!");
				}
			});
			...
			console.log("peerCon after intialize ", peerCon);
		...
		const toggleCamera = () => {
			...
			console.log(
				"condition 3 ",
				!!localStream && typeof localStream !== undefined
			);
		...
		const answerPeer = async (data) => {
			...
			console.log("remote Socket ID is", remoteSocketRef.current);
		...
		const acceptPeer = async (data) => {
			...
			if (implicitAddition) {
				...
				console.log(
					"local stream ref in implicit is ",
					localVideoRef.current.srcObject
				);
				localStream.getTracks().forEach((track) => {
					peerCon.addTrack(track, localVideoRef.current.srcObject);
				});
			}
			setStage("Connected");
		};
		...
		const handleJoinSuccess = async (data) => {
			...
			const isFirst = !!(
				data.offer === null && data.remoteSocket === null
			); // change to checking if there was remote Socket ID or offer or not
						...
						{peer && stage !== "Connected" && (
							...
							>
								<button
									...
									} min-w-[150px] text-xl font-normal`}
									onClick={() => startCall()}
								>
									{stage}
								</button>
