...
const Room4 = () => {
    ...
    */
			...
			if (peerCon !== undefined) {
				console.log("peer COnn is", peerCon);
				setPeer(true);
			}
		...
		const sendStreams = () => {
			console.log("adding tracks...", peerCon);
			if (!peerCon) return;
		...
		const answerPeer = async (data) => {
			...
			const final_peer = await peerCon.setRemoteDescription(data.offer);
			console.log("final peer is ", final_peer);
			const answer = await getAnswer();
			const final_peer2 = await peerCon.setLocalDescription(answer);
		...
		const acceptPeer = async (data) => {
			...
			await peerCon?.setRemoteDescription(data.answer);
		...
		return (
            ...
            /> */}
						...
						{peer && stage !== "Connected" && (
							...
							>
								<button
									className={`bg-blue-600 px-6 py-2 rounded-lg cursor-not-allowed min-w-[150px] text-xl font-normal`}
								>
									{stage}
								</button>
							</div>
