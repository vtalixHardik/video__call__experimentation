...
const Login2 = ({appointment_id, name, report, transcript}) => {
  try {
    console.log("Login2 component rendered report.current is ", report.current);
    console.log("Login2 component rendered transcript.current is ", transcript.current);
    ...
    const [loginTranscript, setLoginTranscript] = useState(transcript.current);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 w-full md:w-3/4 h-fit bg-gray-200 p-4 rounded-xl">
          <div className=" p-4 border border-solid border-x-0 border-t-0 lg:border-y-0 lg:border-l-0 lg:border-x-1">
            ...
            <ul className="pl-5 text-xl">
              <li className=" list-disc">See if you can use two device to avoid a bug</li>
            <li className=" list-disc">If two user from same device join the Video call,</li>
            <li className=" list-disc">then the last role that entered, there `socket instance` will be sent even for the first user that entered(When Join button is pressed)</li>
            <li className=" list-disc">You will see User 1 entering twice</li>
            <li className=" list-disc">
                To avoid this bug for the time being, from a single device make a user join the room(When Join button is pressed) before the other user passes Login
            </li>

            <li className=" list-disc text-red-600"> If you open the website on two tabs before making one user join the video call, It can result in a bug where first user&apos;s socket ID will be used even for second user by the computer</li>
            </ul>
