...
const Room2 = () => {
  ...
  const dataChannel = peerConnectionRef.current.createDataChannel("chat", {negotiated: true, id: 0});
  dataChannelRef.current = dataChannel;
  console.log("data channel created:", dataChannel);
  console.log("data channel ref is ", dataChannelRef.current);

  dataChannel.onopen = () => console.log("Data Channel Opened!");
  dataChannel.onmessage = (event) => {
    console.log("Message received:", event.data);
    setChats((prev) => [...prev, { sender: "Remote", message: event.data }]);
  };

  peerConnectionRef.current.ondatachannel = (event) => {
    console.log("Data Channel received:", event.channel);
    dataChannelRef.current = event.channel;
    console.log("2 data channel ref is ", dataChannelRef.current);

    event.channel.onmessage = (ev) => {
      console.log("Message received:", ev.data);
      setChats((prev) => [...prev, { sender: "Remote", message: ev.data }]);
    };
  };
    ...
    peerConnectionRef.current.addEventListener("track", async (ev) => {
      ...
      if (ev.streams.length > 0) {
        ...
        console.log("remote video ref after set is ", remoteVideoRef.current.srcObject);
  ...
  */
  ...
  useEffect(() => {
    ...
    const controlsHeight = 80; // Default height of controls div (update if different)
    setPosition({ x: window.innerWidth - 450, y: window.innerHeight - controlsHeight - 20 - 275 });
  ...
  }, []); // Dependency array contains `navigate` and `stream`
    ...
    <div className="flex justify-start items-center w-screen h-screen bg-[#F9F4F1] p-1 rounded-xl overflow-hidden">
      ...
      className="grow h-full flex flex-col items-center justify-start relative">
        ...
        style={{
          ...
          className={`absolute bottom-20 xl:bottom-[175px] right-6 w-[276px] xl:w-96 min-h-[207px] max-h-fit h-[207px] rounded-4xl overflow-hidden text-black bg-red-600`}
        >
          <video
            draggable="false"
            ref={localVideoRef}
        ...
        <div
          className={`w-full  ${
            connection === "connected" ? "bg-green-600" : "bg-[#9DA5B8]"
          } h-[80px] xl:h-[150px] flex justify-center items-center gap-12`}
        >
          <div
            className={`xl:h-[75.5px] xl:w-[75.5px] p-5 rounded-full cursor-pointer ${
          ...
          <div
            className={`xl:h-[75.5px] xl:w-[75.5px] text-red-600 p-5 rounded-full cursor-pointer ${
