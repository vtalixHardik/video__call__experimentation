  ...
  try {
    ...
    const initializePeerConnection = async () => {
      ...
      peerCon.addEventListener("track", (ev) => {
        ...
        console.log("Remote video srcObject set before:", remoteVideoRef.current.srcObject);
        // Ensure srcObject is set properly
        if (!remoteVideoRef.current.srcObject || remoteVideoRef.current.srcObject.id !== remoteStream.id) {
          console.log("!remoteVideoRef.current.srcObject || remoteVideoRef.current.srcObject.id !== remoteStream.id");
          remoteVideoRef.current.srcObject = remoteStream;
        }
      
        console.log("Remote video srcObject set after:", remoteVideoRef.current.srcObject);
    ...
    const answerPeer = async (data) => {
      ...
      await peerCon.setRemoteDescription(new RTCSessionDescription(data.offer));
    ...
    <>
      <div className="h-screen w-screen p-2 flex justify-center items-center bg-[#F9F4F1] overflow-hidden">
        ...
        {peer && stage !== "Connected" && (
          ...
          >
            <button className={`bg-blue-600 px-6 py-2 ${stage === "Join" ? "rounded-full cursor-pointer" : "rounded-lg cursor-not-allowed"} min-w-[150px] text-xl font-normal`}
            onClick={()=>startCall()}
            >{stage}</button>
          </div>
          )}
